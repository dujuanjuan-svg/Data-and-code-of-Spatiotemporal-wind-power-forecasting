...
signal=dataset(:,3);
%% Problem Definition
VMDVMD=@(x) VMDVMD(signal,x);      % Cost Function
nVar=3;             % Number of Decision Variables
VarSize=[1 nVar];   % Size of Decision Variables Matrix

VarMin=[1000,0.1,10];          % Lower Bound of Variables
VarMax=[3000,0.2,15];          % Upper Bound of Variables
%% MOPSO Parameters
MaxIt=5;           % Maximum Number of Iterations
nPop=6;            % Population Size
nRep=4;            % Repository Size
...
for i=1:nPop   
    ...
    [Rabbits(i).Cost, Rabbits(i).Sol] = VMDVMD(Rabbits(i).Location);    
end
...
%                 X(i,:)=X_rand.Location-rand()*abs(X_rand.Location-2*rand()*Rabbits(i).Location);
                Rabbits(i).Location=X_rand.Location-rand()*abs(X_rand.Location-rand()*Rabbits(i).Location);
                X(i,:)=X_rand.Location-rand()*abs(X_rand.Location-rand()*Rabbits(i).Location);
            elseif q>=0.5
...
%                 X(i,:)=(leader.Location-mean(X))-rand()*((VarMax-VarMin)*rand+VarMin);
        ...
        elseif abs(Escaping_Energy)<1
            ...
            if r<0.5 && abs(Escaping_Energy)>=0.5 % Soft besiege % rabbit try to escape by many zigzag deceptive motions
                ...
                X1.Location=leader.Location-Escaping_Energy*abs(Jump_strength*leader.Location-X(i,:));
                        X1.Location = max(X1.Location, VarMin);
                        X1.Location = min(X1.Location, VarMax);
                [X1.Cost, X1.Sol] = VMDVMD(X1.Location);
                ...
                elseif Dominates(Rabbits(i),X1)
                    X2.Location=leader.Location-Escaping_Energy*abs(Jump_strength*leader.Location-X(i,:))+rand(1,nVar).*Levy(nVar);
                        X2.Location = max(X2.Location, VarMin);
                        X2.Location = min(X2.Location, VarMax);
                    [X2.Cost, X2.Sol] = VMDVMD(X2.Location);
        ...
        if rand<pm
            ...
            [NewSol.Cost, NewSol.Sol]=VMDVMD(NewSol.Location);
